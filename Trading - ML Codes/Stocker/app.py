...
RATE_LIMIT_COOLDOWN = {}
# Sentiment analyzer (thread-safe for our simple usage)
...
def _sentiment_score(*parts: str):
    """Return sentiment in range [-100, 100] using VADER over concatenated text."""
    ...
    try:
        ...
        logger.debug(f"Sentiment failed for text '{text[:40]}...': {e}")
        return None
...
def run_prediction(symbol: str):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Company info fetch failed for {symbol}: {ie}")
